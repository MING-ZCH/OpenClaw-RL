    ...
    def add_slime_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                choices=["lifo_staleness", "fifo_staleness", "priority", "hybrid", "random", "reservoir", "custom", "per", "prioritized_replay"],
        ...
        def add_algo_arguments(parser):
            ...
            # ============================================================== #
            #  Off-Policy v2: PER / DAPO / TOPR (default OFF, additive flags)  #
            # ============================================================== #
            # --- PER (Prioritized Experience Replay) ---
            parser.add_argument("--per-alpha", type=float, default=0.6,
                help="PER priority exponent. p_i = (|advantage|+eps)^alpha. Default 0.6.")
            parser.add_argument("--per-beta-start", type=float, default=0.4,
                help="PER IS-correction beta at training start. Default 0.4.")
            parser.add_argument("--per-beta-end", type=float, default=1.0,
                help="PER IS-correction beta at full anneal. Default 1.0.")
            parser.add_argument("--per-beta-anneal-steps", type=int, default=1000,
                help="Number of rollout/policy-version steps over which beta linearly anneals start->end.")
            parser.add_argument("--per-priority-source", type=str, default="advantage",
                choices=["advantage", "reward", "reward_dev"],
                help="Source of PER priority signal. Default 'advantage'.")
            parser.add_argument("--per-priority-eps", type=float, default=1e-3,
                help="Additive epsilon on PER priorities to guard zero values.")
            parser.add_argument("--per-min-priority", type=float, default=1e-6,
                help="Floor priority assigned to new/unscored samples.")
            parser.add_argument("--per-max-priority", type=float, default=1e3,
                help="Ceiling clip on PER priorities (numerical safety).")

            # --- DAPO Dynamic Sampling (group-quality admission) ---
            parser.add_argument("--enable-dynamic-sampling", action="store_true", default=False,
                help="DAPO dynamic sampling: reject groups with reward std below threshold. Default OFF.")
            parser.add_argument("--dynamic-sample-min-std", type=float, default=1e-4,
                help="Minimum reward std for a group to enter the buffer. Default 1e-4.")
            parser.add_argument("--dynamic-sample-min-correct-lo", type=int, default=None,
                help="Optional lower bound on n_correct per group (None = disabled).")
            parser.add_argument("--dynamic-sample-min-correct-hi", type=int, default=None,
                help="Optional upper bound on n_correct per group (None = disabled).")

            # --- TOPR sequence-level IS + dual-clip support ---
            parser.add_argument("--use-topr", action="store_true", default=False,
                help="Replace token-level IS weight with TOPR sequence-level w_seq. Default OFF.")
            parser.add_argument("--topr-logw-cap", type=float, default=2.0,
                help="Clamp |log w_seq| <= cap before exp. Default 2.0.")
            parser.add_argument("--topr-w-min", type=float, default=0.0,
                help="Lower clip on TOPR w_seq after exp. Default 0.0 (no floor).")
            parser.add_argument("--topr-w-max", type=float, default=5.0,
                help="Upper clip on TOPR w_seq after exp. Default 5.0.")
            parser.add_argument("--topr-blend", type=float, default=1.0,
                help="Geometric blend lambda in (0,1]; 1.0 = pure TOPR, 0.0 = token-only. Default 1.0.")
            ...
            # === Compatibility shim: --dynamic_history (agenticrl terminal-rl extension) ===
            # slime_offpolicy does not implement dynamic-history logic; accepted
            # so that terminal-rl launchers that include --dynamic_history in
            # GRPO_ARGS do not fail argparse.
            parser.add_argument(
                "--dynamic-history",
                "--dynamic_history",
                ...
                default=False,
                help=(
                    "[Compatibility] Accept --dynamic_history from terminal-rl launchers. "
                    "No-op in slime_offpolicy (feature only implemented in agenticrl branch)."
                ),
            )

            # ── SPEAR Self-Imitation Learning (SIL) flags ─────────────────────────
            # Ref: "Learn the Ropes, Then Trust the Wins" (Qin et al., 2026)
            #      https://github.com/TencentYoutuResearch/SPEAR
            # All flags default OFF; set --enable-trajectory-replay to activate.
            # ─────────────────────────────────────────────────────────────────────
            parser.add_argument(
                ...
                default=False,
                help=(
                    "SPEAR SIL: replay high-score past trajectories as an auxiliary "
                    "self-imitation loss. Admits samples with reward >= "
                    "--trajectory-score-threshold into a FIFO buffer and mixes them "
                    "back into every training batch with a linearly-warmed loss coef."
                ),
            )
            parser.add_argument(
                ...
                type=int,
                default=2048,
                help="SPEAR SIL: max number of trajectories in the self-imitation buffer.",
            )
            parser.add_argument(
                "--baseline-buffer-size",
                type=int,
                default=10240,
                help="SPEAR SIL: (informational) baseline buffer size from SPEAR paper.",
            )
            parser.add_argument(
                "--trajectory-score-threshold",
                type=float,
                default=1.0,
                help="SPEAR SIL: minimum reward for admission (inclusive). Default 1.0.",
            )
            parser.add_argument(
                ...
                default=-1.0,
                help=(
                    "SPEAR SIL: advantage re-estimation mode. "
                    "-1.0 = recompute as A=R-p50(batch); [0,1] = exponential decay."
                ),
            )
            parser.add_argument(
                "--replay-loss-coef",
                ...
                default=0.001,
                help=(
                    "SPEAR SIL: final SIL aux-loss coefficient. "
                    "Actual coef(t) = min(t/max_replay_loss_steps,1.0)*replay_loss_coef."
                ),
            )
            parser.add_argument(
                ...
                type=int,
                default=200,
                help="SPEAR SIL: linear warmup steps for replay_loss_coef (0→final).",
            )
            parser.add_argument(
                "--enable-trajectory-posadv",
                action="store_true",
                default=False,
                help="SPEAR SIL: also require advantage > 0 for buffer admission.",
            )
            # ── end SPEAR SIL ─────────────────────────────────────────────────────
