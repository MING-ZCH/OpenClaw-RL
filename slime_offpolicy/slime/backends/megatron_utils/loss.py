...
def decoupled_policy_loss_function(
    ...
    # === 3.5 Snapshot per-sample lists for TOPR / PER (Off-Policy v2) ===
    # These references stay valid because torch.cat below does not modify the inputs.
    _v2_proximal_log_probs_list = list(proximal_log_probs) if isinstance(proximal_log_probs, list) else None
    _v2_behavior_log_probs_list = list(behavior_log_probs) if isinstance(behavior_log_probs, list) else None
    _v2_loss_masks_list = list(batch.get("loss_masks", [])) if batch.get("loss_masks") is not None else None
    ...
    # === 5.1 TOPR sequence-level IS override (Off-Policy v2, default OFF) ===
    # Gated by --use-topr. Replaces (or blends with) token-level IS weight
    # by per-sample mean log-ratio over response tokens. See slime/utils/topr_utils.py.
    topr_metrics = {}
    if getattr(args, 'use_topr', False) and _v2_proximal_log_probs_list is not None \
            and _v2_behavior_log_probs_list is not None and _v2_loss_masks_list is not None:
        try:
            from slime.utils.topr_utils import compute_topr_seq_weights, blend_token_and_seq_weights
            w_seq_per_sample, w_seq_token = compute_topr_seq_weights(
                _v2_proximal_log_probs_list,
                _v2_behavior_log_probs_list,
                _v2_loss_masks_list,
            ...
            topr_metrics['topr_blend_lambda'] = torch.tensor(
                float(blend), device=importance_weights.device, dtype=torch.float32)
        except Exception as _e:
            # never let v2 features break baseline training
            if is_megatron_main_rank():
                print(f"[TOPR] WARNING: disabled due to error: {_e}")

    # === 5.2 PER per-sample IS weight broadcast (Off-Policy v2, default OFF) ===
    # Gated by --buffer-sampling-strategy=per AND presence of per_is_weights in batch.
    # IS weight w_i = (N * P(i))^(-beta) (normalized) is applied as a scalar
    # multiplier on each sample's loss_mask, which propagates into the
    # token-level PG loss via the existing `sum_of_sample_mean` reducer.
    per_metrics = {}
    _per_is_weights = batch.get('per_is_weights', None)
    if _per_is_weights is not None and _v2_loss_masks_list is not None \
            and len(_per_is_weights) == len(_v2_loss_masks_list):
        try:
            import torch as _torch
            new_masks = []
            for w, m in zip(_per_is_weights, batch['loss_masks']):
                ws = float(w)
                new_masks.append(m * ws)
            batch['loss_masks'] = new_masks
            _w_t = _torch.tensor(list(_per_is_weights), dtype=_torch.float32, device=importance_weights.device)
            per_metrics['per_is_weight_mean'] = _w_t.mean()
            per_metrics['per_is_weight_min'] = _w_t.min()
            per_metrics['per_is_weight_max'] = _w_t.max()
        except Exception as _e:
            if is_megatron_main_rank():
                print(f"[PER] WARNING: IS weight broadcast failed: {_e}")
    ...
    for metric_name, metric_value in topr_metrics.items():
        reported_loss[metric_name] = metric_value if isinstance(metric_value, torch.Tensor) else torch.tensor(float(metric_value), device=log_probs.device)
    for metric_name, metric_value in per_metrics.items():
        reported_loss[metric_name] = metric_value if isinstance(metric_value, torch.Tensor) else torch.tensor(float(metric_value), device=log_probs.device)
