    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch) -> None:
        ...
        with inverse_timer("train_wait"), timer("train"):
            ...
            if self.args.compute_advantages_and_returns:
                ...
                # === SPEAR SIL: Override advantages for self-imitation samples ===
                # After compute_advantages_and_returns() whitens all advantages,
                # we replace SIL sample advantages with pre-computed values
                # (A_sil = R_sil - p50_main_rewards) to avoid contamination from
                # high-reward SIL samples skewing the global whitening statistics.
                if (getattr(self.args, 'enable_trajectory_replay', False)
                        and rollout_data.get('sil_sample_flags')):
                    try:
                        _sil_flags = rollout_data['sil_sample_flags']
                        _sil_pre_advs = rollout_data.get('sil_precomputed_advantages', [])
                        _advantages = rollout_data.get('advantages', [])
                        _n_overridden = 0
                        for _i, _flag in enumerate(_sil_flags):
                            if (_flag == 1
                                    and _i < len(_sil_pre_advs)
                                    and _sil_pre_advs[_i] is not None
                                    and _i < len(_advantages)):
                                _sil_adv = _sil_pre_advs[_i]
                                if hasattr(_sil_adv, "to") and hasattr(_advantages[_i], "device"):
                                    _sil_adv = _sil_adv.to(device=_advantages[_i].device, dtype=_advantages[_i].dtype)
                                _advantages[_i] = _sil_adv
                                _n_overridden += 1
                        if _n_overridden > 0:
                            print(f"[SIL] Overrode advantages for {_n_overridden} "
                                  f"SIL samples (bypassing whitening)")
                    except Exception as _sil_e:
                        print(f"[SIL] WARNING: advantage override failed (non-fatal): {_sil_e}")
                # === end SPEAR SIL advantage override ===
