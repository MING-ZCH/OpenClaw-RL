...
def setup_model_and_optimizer(
    ...
    if no_wd_decay_cond is not None or scale_lr_cond is not None or lr_mult != 1.0:
        raise NotImplementedError(
            "The vendored Megatron optimizer API now expects config_overrides "
            "instead of no_wd_decay_cond/scale_lr_cond/lr_mult."
        )
...
def train_one_step(
    ...
    def forward_step(data_iterator: DataIterator, model: GPTModel, return_schedule_plan: bool = False) -> tuple[
        ...
        if args.loss_type == "decoupled_policy_loss":
            ...
            # === Off-Policy v2 (PER): per-sample importance-sampling weight ===
            # Always include — rollout.py emits 1.0 by default, so this is a
            # no-op when PER is OFF. Loss path treats this as multiplier on loss_masks.
