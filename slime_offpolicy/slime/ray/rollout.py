...
class RolloutManager:
    ...
    def generate(self, rollout_id):
        ...
        try:
            ...
            train_data = self._convert_samples_to_train_data(train_data_samples, rollout_id=rollout_id)
    ...
    def _convert_samples_to_train_data(self, samples: Union[list[Sample], list[list[Sample]]], rollout_id: int = 0):
        """
        ...
        for sample in samples:
            ...
            # === swe-rl bridge patch (zhangchenhao 20260528) ===
            # Honor dynamic `remove_sample` attribute that downstream generate
            # functions (e.g. generate_with_swe_remote.py) set to drop garbage
            # rollouts. PR #7's Sample dataclass does NOT declare this field but
            # Python lets it become a dynamic attribute. Zero its loss_mask so
            # the off-policy buffer/loss path treats it as a no-op.
        ...
        # === Off-Policy v2: PER importance-sampling weights ===
        # PER attaches per-sample IS weight to sample.metadata["per_is_weight"]
        # via PrioritizedReplayStrategy.sample(). Always emit this list (default 1.0)
        # so the loss path (decoupled_policy_loss) can broadcast it onto loss_masks
        # when --buffer-sampling-strategy=per is active. When PER is OFF, every
        # weight is 1.0 and the multiplication in loss.py is a no-op.
        ...
        # === SPEAR SIL: Mix self-imitation samples into batch (default OFF) ===
        # Only active when --enable-trajectory-replay is set and SIL buffer is populated.
        # Samples high-score trajectories and replaces a small tail slice with
        # pre-computed advantages
        # that will override whitened GRPO advantages in actor.py.
        if (getattr(self.args, 'enable_trajectory_replay', False)
                and hasattr(self, 'data_source')
                and getattr(self.data_source, 'sil_buffer', None) is not None
                and len(self.data_source.sil_buffer) > 0):
            try:
                import statistics as _stat
                _sil_buf = self.data_source.sil_buffer
                # 25% of main batch, at least 1, at most buffer size
                _sil_n = max(1, min(len(samples) // 4, len(_sil_buf)))
                # Compute baseline from main batch rewards BEFORE appending SIL samples
                _main_rewards = list(train_data.get("rewards", []))
                _baseline = float(_stat.median(_main_rewards)) if _main_rewards else 0.0
                # Progressive coefficient: linear warmup from 0 to replay_loss_coef
                _max_steps = float(getattr(self.args, 'max_replay_loss_steps', 200))
                _final_coef = float(getattr(self.args, 'replay_loss_coef', 0.001))
                _coef = min(rollout_id / max(_max_steps, 1.0), 1.0) * _final_coef
                # Sample from SIL buffer
                _sil_entries = _sil_buf.sample(_sil_n, rollout_id, _baseline)
                if _sil_entries:
                    _n_main = len(samples)
                    _sil_flags = [0] * _n_main          # main=0, SIL=1
                    _sil_pre_advs = [None] * _n_main    # None for main samples
                    # Keep the global batch size unchanged. Appending SIL samples
                    # creates a remainder that Megatron's fixed global-batch
                    # schedule silently drops; replace a small tail slice instead.
                    _replace_start = _n_main - len(_sil_entries)
                    for _j, _entry in enumerate(_sil_entries):
                        _dst = _replace_start + _j
                        _resp_len = int(_entry["response_length"])
                        _adv_val = float(_entry["advantage"])  # = R_sil - baseline
                        # Pre-compute advantage tensor (will override whitening in actor.py)
                        import torch as _torch
                        _adv_tensor = _torch.full((_resp_len,), _adv_val, dtype=_torch.float32)
                        _sil_pre_advs[_dst] = _adv_tensor
                        _sil_flags[_dst] = 1
                        # Scale loss_mask by progressive coef to control SIL contribution
                        _raw_mask = _entry.get("loss_mask", [1] * _resp_len)
                        _scaled_mask = [float(m) * _coef for m in _raw_mask]
                        # Replace tail samples in-place so data partitioning keeps
                        # SIL samples inside the trainable global batch.
                        train_data["tokens"][_dst] = _entry["tokens"]
                        train_data["response_lengths"][_dst] = _resp_len
                        train_data["loss_masks"][_dst] = _scaled_mask
                        train_data["rewards"][_dst] = float(_entry["reward"])
                        train_data["raw_reward"][_dst] = float(_entry["reward"])
                        train_data["per_is_weights"][_dst] = 1.0
                        train_data["truncated"][_dst] = 0
                        train_data["sample_indices"][_dst] = -1  # marker for SIL
                        # Optional fields: replace matching values if present
                        if "rollout_log_probs" in train_data:
                            train_data["rollout_log_probs"][_dst] = _entry.get("rollout_log_probs")
                        if "policy_versions" in train_data:
                            train_data["policy_versions"][_dst] = 0  # stale behavior policy
                    # Replace any remaining list fields not explicitly handled above
                    # (e.g. round_number, rollout_routed_experts, metadata,
                    # teacher_log_probs) to avoid stale per-sample metadata.
                    _SIL_HANDLED_KEYS = {
                        "tokens", "response_lengths", "loss_masks", "rewards",
                        "raw_reward", "per_is_weights", "truncated", "sample_indices",
                        "rollout_log_probs", "policy_versions",
                    }
                    for _k, _v in train_data.items():
                        if (_k not in _SIL_HANDLED_KEYS
                                and isinstance(_v, list)
                                and len(_v) == _n_main):
                            for _dst in range(_replace_start, _n_main):
                                train_data[_k][_dst] = None
                    train_data["sil_sample_flags"] = _sil_flags
                    train_data["sil_precomputed_advantages"] = _sil_pre_advs
                    train_data["sil_reward_baseline"] = _baseline
                    print(f"[SIL] Mixed {len(_sil_entries)} SIL samples into batch "
                          f"(coef={_coef:.6f}, baseline={_baseline:.4f}, "
                          f"replaced_tail={len(_sil_entries)}, total_batch={len(train_data['tokens'])})")
            except Exception as _sil_e:
                print(f"[SIL] WARNING: mixing failed (non-fatal, continuing without SIL): {_sil_e}")
        # === end SPEAR SIL mixing ===
